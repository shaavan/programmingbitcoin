...
class MerkleBlock:
    ...
    def parse(cls, s):
        ...
        total_trans = little_endian_to_int(s.read(4))
        # number of transaction hashes - varint
        num_trans = read_varint(s)
        hashes = []
        ...
        # initialize class
        return MerkleBlock(version, prev_block, merkle_root, timestamp, bits, nonce, total_trans, hashes, flags)
        raise NotImplementedError

    def is_valid(self):
        '''Verifies whether the merkle tree information validates to the merkle root'''
        # convert the flags field to a bit field
        bit_flags = bytes_to_bit_field(self.flags)
        ...
        merkle_tree.populate_tree(bit_flags, hashes)
